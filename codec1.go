...
// Default block sizes; configured by setBlocksForQuality.
var (
...
// setBlocksForQuality configures global smallBlock/macroBlock based on a 0–100 quality value.
// We use the same bands as canUseBigBlock:
//
//	80–100 → 1x2 blocks (highest quality, smallest macro blocks)
//	60–79  → 1x3
//	40–59  → 2x4
//	20–39  → 3x6
//	 0–19  → 4x8 (lowest quality, biggest blocks)
...
// pickPatternChannel analyzes a block and chooses which channel (R, G, or B)
// produces the best bi-level split into foreground/background.
// It returns the chosen channel index (0=R,1=G,2=B,3=luma fallback) and the threshold for that channel.
func pickPatternChannel(img image.Image, x0, y0, bw, bh int) (int, int32, error) {
	b := img.Bounds()
	if x0 < b.Min.X || y0 < b.Min.Y || x0+bw > b.Max.X || y0+bh > b.Max.Y {
	...
	// First pass: compute average per channel.
	...
	thrR := int32(sumR / uint64(total))
	thrG := int32(sumG / uint64(total))
	thrB := int32(sumB / uint64(total))
	...
	// Second pass: for each channel, split pixels by its threshold and
	// accumulate statistics to judge how well it separates the block.
	for yy := 0; yy < bh; yy++ {
		for xx := 0; xx < bw; xx++ {
			c := img.At(x0+xx, y0+yy)
			r16, g16, b16, _ := c.RGBA()
			...
			vals := [3]uint8{r8, g8, b8}
			thrs := [3]int32{thrR, thrG, thrB}

			for ch := 0; ch < 3; ch++ {
				v := int32(vals[ch])
				if v >= thrs[ch] {
					st[ch].fgSum += uint64(vals[ch])
					st[ch].fgCount++
				} else {
					st[ch].bgSum += uint64(vals[ch])
					st[ch].bgCount++
				}
			}
		}
	...
	bestCh := -1
	var bestSpread int32
	for ch := 0; ch < 3; ch++ {
		if st[ch].fgCount == 0 || st[ch].bgCount == 0 {
			continue
		}
		fgMean := int32(st[ch].fgSum / uint64(st[ch].fgCount))
		bgMean := int32(st[ch].bgSum / uint64(st[ch].bgCount))
		spread := fgMean - bgMean
		if spread < 0 {
			spread = -spread
		}
		if bestCh < 0 || spread > bestSpread {
			bestCh = ch
			bestSpread = spread
		}
	}
	...
	if bestCh < 0 {
		// Completely flat or degenerate block: just pick R-channel.
		bestCh = 0
	}
...
func encodeBlock(img image.Image, x0, y0, bw, bh int, getIndex func(rgb3) (uint16, error), pw *BitWriter) (uint16, uint16, error) {
	channel, threshold, err := pickPatternChannel(img, x0, y0, bw, bh)
	if err != nil {
		return 0, 0, err
	}
...
// It computes a per-channel threshold and writes per-pixel pattern bits, but does not compute or use palette indices.
...
func Encode(img image.Image, quality int) ([]byte, error) {
	...
	for my := 0; my < fullH; my += macroBlock {
		for mx := 0; mx < fullW; mx += macroBlock {
			if useMacro && canUseBigBlock(img, mx, my, quality) {
...
// The decision is based on the luminance range and the requested quality:
// within each quality band (0–19, 20–39, 40–59, 60–79, 80–100) we linearly
// scale the allowed spread from 64 (more macro blocks) down to 8 (fewer macro blocks).
func canUseBigBlock(img image.Image, x0, y0, quality int) bool {
	b := img.Bounds()
	...
	// Helper: for a given band [qStart, qEnd], linearly interpolate spread from 64 → 8.
	lerpSpread := func(q, qStart, qEnd int) int32 {
		if q <= qStart {
			return 64
		}
		if q >= qEnd {
		...
		t := float64(q-qStart) / float64(qEnd-qStart)
		return int32(64 + (8-64)*t + 0.5)
	}

	var maxSpread int32
	switch {
	case quality <= 19:
		maxSpread = lerpSpread(quality, 0, 19)
	case quality <= 39:
		maxSpread = lerpSpread(quality, 20, 39)
	case quality <= 59:
		maxSpread = lerpSpread(quality, 40, 59)
	case quality <= 79:
		maxSpread = lerpSpread(quality, 60, 79)
	default:
		maxSpread = lerpSpread(quality, 80, 100)
	}
